...
Note: The command ```npm run dev``` runs a local server on your machine that hosts the site. The command ```npm run dev``` runs a mock api that hosts all of the data for the assessment.  
